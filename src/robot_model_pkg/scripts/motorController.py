...
ser = serial.Serial(
    # change port (check by: ls /dev/tty*)
    port = '/dev/ttyAMA0',
    baudrate = 9600,
...
# new
# Send velocity (rpm) to stm32
    dataSendUart = (str(right_rpm) + "r" + str(left_rpm) + "l").encode("utf-8")
    ser.write(dataSendUart)
    ser.flush()
...
def listener():
    ...
    rospy.init_node('uartControl', anonymous=True)
